...
function showSlides(n, no) {
  ...
  setTimeout(() => {
    for (i = 0; i < yesElements.length; i++) {
      yesElements[i].style.backgroundColor = " #ffbd43"; // Revert to grey
      yesElements[i].style.color = "white";
    }
    for (i = 0; i < noElements.length; i++) {
      noElements[i].style.backgroundColor = " #ffbd43"; // Revert to grey
    }
    //enable
    allButtons.forEach((button) => {
...
function back() {
  if (m > 1) {
    // Check if user has navigated from the starting page
    showSlides((slideIndex[0] = 1), 0); // Go back to the starting page
  } else {
    // If already on the starting page, prevent further back navigation
